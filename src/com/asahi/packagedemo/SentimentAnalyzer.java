...
public class SentimentAnalyzer {
	...
	public static int[] detectProsAndCons(String review, String[][] featureSet, String[] posOpinionWords,
        ...
        for(i=0; i<featureSet.length; i++) {
        	...
        	if(featureSet[i] != null) {
        		
	        	for(j=0; j<featureSet[i].length; j++) {
	        		       		
	        		int val = getOpinionOnFeature(review, featureSet[i][j], posOpinionWords , negOpinionWords );
	        		    
	        		if(val == 1) {	
	        		  featureOpinions[k] = val;
	        		  k++;
	        		  if(k > i) {
	        			break;  
	        		  }
	        		  if(i < k) {
	        			...
	        			if(j == (featureSet[i].length-1)) {		
	        				if(k <= featureSet.length) {
	        					featureOpinions[k] = val;
	        					k++;
	        				}
	        			}
	...
	private static int getOpinionOnFeature(String review, String feature, String[] posOpinionWords, String[] negOpinionWords) {
		...
		// your code
		checkForWasPhrasePattern(review, feature, posOpinionWords, negOpinionWords);
		
		String smallLetterReview = review.toLowerCase();
		boolean getChar = smallLetterReview.contains(feature);
		int val = getChar ? 1 : 0;
	...
	public static void main(String[] args) {
		...
		System.out.println("Opinions on Features: "+ featureOpinions.length + Arrays.toString(featureOpinions));
	}
}
